	...
	describe('shortest route', () => {
		...
		it(
			...
			async () => {
				...
				const response = await request(server).get('/routes/HAV/TAY?max-flights-count=5');
				const body = response.body;
		...
		it(
			...
			async () => {
				...
				const response = await request(server).get('/routes/HAV/TAY?max-flights-count=3');
				const body = response.body;
		...
		it(
			...
			async () => {
				const allowedHops = 2;
				const response = await request(server).get(`/routes/HAV/TAY?max-flights-count=${allowedHops}`);
				const body = response.body;
